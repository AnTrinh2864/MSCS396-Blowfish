//MSCS 396 Project Blowfish
//This is our implementation on Blowfish Algorithm
import java.util.*;
public class BlowFish2 {
    ...
    String cipherText = "";
    String plainText = "";
    static String tmpStore = new String();
    static String finalStore = new String();
    static double enTime = 0;
    static double deTime = 0;
    ...
    private String round(int time, String plainText)
    {
        ...
        right = plainText.substring(plainText.length()/2, plainText.length());
    ...
    private String encrypt(String plainText)
    {
        ...
        // postprocessing
        String right = plainText.substring(0, plainText.length()/2);
        String left = plainText.substring(plainText.length()/2, plainText.length());
        right = xor(right, P[16]);
        left = xor(left, P[17]);
        cipherText += left + right;
        return left + right;
    }
    //Final Blowfish implementation
    BlowFish2(String plainText)
    {
        for (int i = 0; i < 32; i++)
            modVal = modVal << 1;
        char[] list = plainText.toCharArray();
        String text = "";
        for (int i = 0; i < list.length; i++){
            text += Integer.toHexString(list[i]);
        }
        if (text.length() < 16) {
        ...
        System.out.println("Encryption took: " + (end - start)*1000/1e9 + " ms");
        enTime += (end - start)*1000/1e9;
        ...
        System.out.println("Decryption took: " + (end - start)*1000/1e9 + " ms");
        System.out.println("Plain Text: "
                + plainText);
        deTime += (end-start)*1000/1e9;

        //System.out.println("Plain text length: " + plainText.length());
        tmpStore = new String(hexStringToByteArray(plainText));
        String tmpFinal = new String(tmpStore.replaceAll("\\P{Print}", ""));
        System.out.println("Convert Back: " + tmpStore.replaceAll("\\P{Print}", "")); //Receive a function to
        finalStore += new String (tmpFinal);
    }
    //decryption
    ...
    public static void main(String args[])
    {
        ...
        int count = 0;
            for (int i = 0; i< text.length(); i+= 8) {
                int endIndex = Math.min(i + 8, text.length());
                String block = text.substring(i, endIndex);
                System.out.println(block);
                list[count] = block;
                count++;
            }

            System.out.println("Total blocks: " + count);
            for (int i = 0; i < count; i++) {
                System.out.println("plain text: " + list[i]);
                new BlowFish2(list[i]);
                System.out.println();
            }
        System.out.println("Previous Message: " + finalStore.substring(0,text.length()));
        System.out.println("Total time for encryption: " + enTime + "ms");
        System.out.println("Total time for decryption: " + deTime + "ms");
    }//end main
}
